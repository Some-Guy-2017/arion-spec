...
The following is a class diagram showcasing the breakdown of classes, their methods, method parameters and return types, and how the classes interact.
...
The following is a sequence diagram depicting the method calls for the Load feature.
First, the user clicks the Load menu item.
...
It then returns these flashcards, which are written into memory.
\image{load-sd}[0]

\subsubsection*{Save Sequence Diagram}
The following describes the method calls for the Save feature.
It follows a similar path as the Load functionality;
...
This sequence diagram shows the method calls involved in executing the Browse feature.
After the menu item is clicked and the corresponding menuCallback is run, Arion calls ArionDisplay to display the browse screen.
It passes the flashcards, since these are displayed in the screen, and an edit and delete callback.
When ArionDisplay adds the delete and edit buttons, it sets both of their ActionListeners to perform these callbacks,
